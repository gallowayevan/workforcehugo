(() => {
  ...
  // src/config.js
  var useLocalApi = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("api") === "local";
  var rootApi = useLocalApi ? "http://localhost:1313/" : "https://hpdsdatanode-dept-healthworkforce.apps.cloudapps.unc.edu/";
  ...
  async function loadGeo() {
    const names = ["ncMap", "ahec", "medicaid", "ruralRoots", "physicianGroups"];
    ...
    const [ncMap, ahec, medicaid, ruralRoots, physicianGroups] = results;
    return { ncMap, ahec, medicaid, ruralRoots, physicianGroups, layers: null };
  }
  async function loadLayersGeo() {
    const r = await fetch("./data/layers.json");
    if (!r.ok) throw new Error("Failed to load data/layers.json");
    return r.json();
  }
  ...
  var ahecRegionName = /* @__PURE__ */ new Map([
    ["Greensboro", "Piedmont"],
    ["Charlotte", "South Piedmont"]
  ]);
  ...
  function displayRegion(level, region) {
    if (level == "medicaid") return medicaidRegionName.get(region) || region;
    if (level == "ahec") return ahecRegionName.get(region) || region;
    return region;
  }
  var variableLabels = {
  ...
  async function changeSpecialty(specialtyObject) {
    ...
    try {
      ...
      if (!Array.isArray(raw) || raw.length === 0) {
        throw new Error(`Empty dataset for specialty ${specialtyObject.code}`);
      }
      let data = raw;
      let yearMin = 4e3;
      let yearMax = 0;
      ...
      data.forEach(function(d) {
        d.total = d.total === null ? null : +d.total;
        d.year = +d.year;
        d.provider_rate = d.provider_rate === null ? null : +d.provider_rate;
  ...
  var layersGeoPromise = null;
  async function updateLayers(layers) {
    state.layers = layers;
    emit();
    if (layers.length && state.geo && !state.geo.layers) {
      try {
        layersGeoPromise = layersGeoPromise || loadLayersGeo();
        state.geo.layers = await layersGeoPromise;
        emit();
      } catch (error) {
        layersGeoPromise = null;
  ...
  function buildColorScale(state2) {
    ...
    const flatScale = () => threshold().domain([0, Number.EPSILON]).range(["url(#NApattern)", "#e3e3e3", scheme[4][3]]);
    if (currDomain.length === 0) return flatScale();
    if (state2.aggregationLevel == "county") {
      const hist = histogramData(state2);
      if (hist.length < 2) return flatScale();
  ...
  function renderLayers(state2) {
    if (!state2.geo.layers) return `<g class="layers" pointer-events="none"></g>`;
  ...
  function renderHistogram(state2, colorScale, histData, mapData) {
    ...
    const selVal = state2.region != "North Carolina" ? mapData.get(state2.region) : void 0;
    const selClass = (isSel) => isSel ? " selected-bin" : "";
    ...
    const countLabel = "Counties";
    const zeroBin = `<g transform="translate(${xScale.range()[0] - BIN_WIDTH * 1.9}, ${yScale(0)})">
    <rect class="bin${selClass(selVal === 0)}" x="0" y="${yScale(zeroRegions) - yScale(0)}" width="${BIN_WIDTH}" height="${yScale(0) - yScale(zeroRegions)}" fill="${colorScale(0)}" data-tip="${zeroRegions} counties"></rect>
    ...
    const bars = histData.map((item, index2) => {
      ...
      const isSel = selVal != null && selVal > 0 && selVal >= item.x0 && (index2 == histData.length - 1 ? selVal <= item.x1 : selVal < item.x1);
      return `<rect class="bin${selClass(isSel)}" x="${x2}" y="${yScale(item.length)}" width="${BIN_WIDTH}" height="${yScale(0) - yScale(item.length)}" fill="${colorScale(item.x0)}" data-tip="${item.length} counties"></rect>`;
    }).join("");
    const naBin = isPercent ? `<g transform="translate(${xScale.range()[1] + BIN_WIDTH * 0.9}, ${yScale(0)})">
        <rect class="bin${selClass(selVal != null && selVal < 0)}" x="0" y="${yScale(missingRegions) - yScale(0)}" width="${BIN_WIDTH}" height="${yScale(0) - yScale(missingRegions)}" fill="${colorScale(-9)}" data-tip="${missingRegions} counties"></rect>
        <line x2="${BIN_WIDTH}" x1="0"></line>
  ...
  function renderRowChart(state2, colorScale, mapData) {
    ...
    const labelFor = (region) => region == "Wake AHEC" ? "Wake" : state2.aggregationLevel == "ruralroots" ? ruralRootsShort[region] || region : displayRegion(state2.aggregationLevel, region);
    const axisValueFormatter = (v) => {
      if (isPercent) return Math.round(v * 100) + "%";
      if (variable == "provider_rate") return format("")(v);
      return valueFormatter(v);
    ...
    const ncText = ncRow ? `${stateLabel}: ${valueFormatter(ncRow.value)}` : "";
    const rows = mapDataArray.map(([region, value]) => {
      const selected = region == state2.region ? " selected-row" : "";
      return `<g class="row${selected}" transform="translate(0,${yScale(region)})">
        <rect width="${Math.max(0, xScale(+value))}" height="${yScale.bandwidth()}" fill="${colorScale(+value)}" data-tip="${hoverText(+value)}"></rect>
        <text class="ahecLabel" dx="-10" dy="${yScale.bandwidth() / 1.5}">${escapeHtml(labelFor(region))}</text>
      </g>`;
    }).join("");
  ...
  // src/charts/map.js
  var MAP_VIEWBOX = "-15 48 655 562";
  var _pathCache = /* @__PURE__ */ new WeakMap();
  function pathFor(path2, feature) {
    let d = _pathCache.get(feature);
    if (d === void 0) {
      d = path2(feature.geometry);
      _pathCache.set(feature, d);
    }
    return d;
  }
  ...
  function overlayRegionLabel(level, region) {
    if (region == "Wake AHEC") return "Wake";
    return displayRegion(level, region);
  }
  function mapTitleText(state2) {
    return `${professionChartTitle(state2.variable, state2.specialty)} by ${aggLabel[state2.aggregationLevel]}, North Carolina, ${state2.year}`;
  }
  var PHYSICIAN_UNAVAILABLE_TEXT = "Physician data from 2020 and 2021 are unavailable.";
  function physicianDataUnavailable(state2) {
    return state2.specialty.profession === "Physician" && (state2.year == 2020 || state2.year == 2021);
  }
  function renderMap(state2, { includeTitle = false } = {}) {
    const { ncMap } = state2.geo;
    if (physicianDataUnavailable(state2)) {
      return `<text x="313" y="290" text-anchor="middle" font-size="22px" fill="#000">${PHYSICIAN_UNAVAILABLE_TEXT}</text>`;
    }
    const path2 = getPath(ncMap);
    ...
    const title = includeTitle ? `<text id="map-title" transform="translate(0,25)">${wrap({
      text: mapTitleText(state2),
      maxCharsPerLine: 85
    })}</text>` : "";
...
${hoverValueText(state2, val)}${showTotalInTip && total != null ? `
(${total} total)` : ""}`;
      return `<path class="county${selected}" data-region="${escapeHtml(county)}" data-tip="${escapeHtml(tip)}" fill="${colorScale(val)}" d="${pathFor(path2, f)}"></path>`;
    }).join("");
...
${hoverValueText(state2, val)}${showTotalInTip && total != null ? `
(${total} total)` : ""}`;
        return `<path class="overlay${selected}" data-region="${escapeHtml(r)}" data-tip="${escapeHtml(tip)}" d="${pathFor(path2, f)}"></path>`;
      }).join("");
    ...
    const histData = level == "county" ? histogramData(state2) : null;
    const legendChart = level == "county" ? histData.length >= 2 ? renderHistogram(state2, colorScale, histData, mapData) : "" : renderRowChart(state2, colorScale, mapData);
    const defs = `<defs>
  ...
  var SPARK_VIEWBOX = "-30 0 372 710";
  var WIDTH3 = 240;
  ...
  function headerRegion(state2) {
    ...
    if (region == "North Carolina") return region;
    if (lvl == "county") return `${region} County`;
    if (lvl == "ahec" && region.indexOf("AHEC") == -1) return `${region} AHEC`;
    return displayRegion(lvl, region);
  }
  function sparkChart(state2, variable) {
    ...
    const bars = rects.map((r) => {
      ...
      const selected = r.year == state2.year ? " is-selected" : "";
      return `<rect class="spark-bar${selected}" data-year="${r.year}" x="${r.x}" y="${r.y}" width="${r.width}" height="${r.height}" style="fill:${fill}"></rect>`;
    }).join("");
    let nc = "";
    if (state2.region != "North Carolina") {
      ...
      const tx = variable.indexOf("percent") > -1 ? ncx + 20 : ncx + 10;
      const lastRect = rects[n - 1];
      let ty = ncy + 5;
      if (lastRect && Math.abs(ty - (lastRect.y - 4)) < 14) {
        ty = lastRect.y + 12;
      }
      nc = `<g transform="translate(${MARGIN3.left}, ${MARGIN3.top})">
      <path class="spark-ncline" d="${linePath}"></path>
      <text class="spark-nctext" x="${tx}" y="${ty}">${txt}</text>
    </g>`;
    }
  ...
  function sparksMarkup(state2) {
    const header = `<text class="spark-header" transform="translate(0,-65)">${wrap(
      {
        text: escapeHtml(`Profession Demographics for ${headerRegion(state2)}`),
        maxCharsPerLine: 30,
        lineHeight: 1.1
      }
    )}</text>`;
  ...
  .histogram-legend .bin.selected-bin, .row-chart .selected-row rect { stroke: rgb(163,49,128); stroke-width: 2.5; }
  .row-chart .selected-row text { font-weight: 700; }
  ...
  .spark-bar.is-selected { stroke: #4a4a4a; stroke-width: 1.5; }
...
`;
  ...
  function exportFilename(state2, suffix, ext) {
    const slug = (s) => String(s).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
    return `nc-supply-${slug(state2.specialty.name)}-${state2.aggregationLevel}-${suffix}.${ext}`;
  }
  ...
  async function downloadImage() {
    ...
    let content = `<style>${EXPORT_STYLE}</style><rect width="${W}" height="${H}" fill="#fff" /><g transform="translate(25,25)">${renderMap(state2, { includeTitle: true })}</g><g transform="translate(720,25)">${sparksMarkup(state2)}</g><text font-size="10px" transform="translate(10,750)">${wrap({
      text: notesText,
      maxCharsPerLine: 200
    })}</text>`;
    try {
    ...
    try {
      ...
      triggerDownload(blob, exportFilename(state2, state2.year, "png"));
    } catch (err) {
      console.error("Image export failed:", err);
      const status = document.getElementById("hw-status");
      if (status)
        status.textContent = "Sorry \u2014 the image export failed. Please try again.";
    }
  }
  function downloadCsv() {
    ...
    const formattedData = Array.from(
      ...
      let newRow = {};
      newRow[aggregationLevel] = aggregationLevel == "ahec" ? ahecRegionName.get(d[0]) || d[0] : d[0];
...
`;
    ...
    triggerDownload(blob, exportFilename(state2, variable.replace(/_/g, "-"), "csv"));
  }
  ...
  <ul>
    ...
    <li>Use the <strong>year</strong> slider to change the mapped year. Hover or click the small charts on the right to read any year; the blue line marks the statewide value.</li>
    <li>In the small charts, <strong>purple</strong> bars trace the measure currently shown on the map (green bars are the other measures), and the outlined bar is the mapped year.</li>
  ...
  function renderViewControls(container, state2) {
    ...
    ].map((r) => ({ value: r, label: displayRegion(state2.aggregationLevel, r) })).sort((a, b) => a.label > b.label ? 1 : a.label < b.label ? -1 : 0);
    ...
    const layerBoxes = LAYERS.map(
      (l) => `<label class="layer-check"><input type="checkbox" id="hw-layer-${l.value}" data-control="layer" value="${l.value}"${active.has(l.value) ? " checked" : ""} /> ${l.label}</label>`
    ).join("");
    container.innerHTML = `
    <div class="control-field">
      ...
      <input id="hw-year" type="range" data-control="year"
        ...
        value="${state2.year}" />
      <div class="range-bounds" aria-hidden="true"><span>${state2.yearExtent[0]}</span><span>${state2.yearExtent[1]}</span></div>
    </div>
    <div class="control-field">
    ...
    <div class="control-actions">
      ...
      <button type="button" class="hw-btn" data-action="help" aria-expanded="${isHelpOpen()}" aria-controls="hw-help">How to use this</button>
    </div>`;
  }
  ...
  <div class="hw-app">
    ...
    <div id="hw-dashboard" class="dashboard">
      <div class="map-cell">
        <h2 id="hw-map-title" class="map-title"></h2>
        <svg id="hw-map" class="map-svg" viewBox="${MAP_VIEWBOX}" role="img"
             aria-label="Choropleth map of North Carolina health workforce supply"></svg>
      </div>
      <div class="sparks-cell">
        <svg id="hw-sparks" class="spark-svg" viewBox="${SPARK_VIEWBOX}" role="img"
    ...
    <p id="hw-announce" class="sr-only" aria-live="polite"></p>
  ...
  var mapTitleEl = document.getElementById("hw-map-title");
  ...
  var announceEl = document.getElementById("hw-announce");
  ...
  function render(state2) {
    ...
    if (sig !== lastControlsSig) {
      ...
      lastControlsSig = sig;
      if (focusId) (_a = document.getElementById(focusId)) == null ? void 0 : _a.focus();
    } else {
      updateYearUI(state2);
    }
    if (state2.loadFailed) {
      dashboardEl.hidden = true;
      statusEl.textContent = `Sorry \u2014 data for ${state2.specialty.name} couldn't be loaded right now. Please try another profession, or check your connection and try again.`;
      return;
    }
    ...
    hideTooltip();
    const title = mapTitleText(state2);
    const unavailableNote = physicianDataUnavailable(state2) ? ` ${PHYSICIAN_UNAVAILABLE_TEXT}` : "";
    const regionNote = state2.region != "North Carolina" ? ` ${displayRegion(state2.aggregationLevel, state2.region)} selected.` : "";
    mapTitleEl.textContent = title;
    mapEl.setAttribute("aria-label", `Map: ${title}.${unavailableNote}`);
    sparksEl.setAttribute(
      "aria-label",
      `Trend charts: profession demographics for ${headerRegion(state2)}, ${state2.yearExtent[0]} to ${state2.yearExtent[1]}`
    );
    announceEl.textContent = `${title}.${unavailableNote}${regionNote}`;
  }
  ...
  function scheduleRender() {
    ...
    frameId = requestAnimationFrame(() => {
      ...
      try {
        ...
        console.error(err);
        statusEl.textContent = "Sorry \u2014 something went wrong drawing this view. Try a different selection, or reload the page.";
      }
    });
  }
  initControls(appEl);
  ...
  (async function init() {
    try {
      ...
      const start = professions[Math.floor(Math.random() * professions.length)];
      render(getState());
